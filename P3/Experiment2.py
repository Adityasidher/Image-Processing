...
def DFT2D():
    ...
    image = Image.open("./data_input/generatedImg32.pgm")
    ...
    newImage.save("./data_output/Experiment2/partxtra/Potato.png")
...
def ApplyFFTRow(pixels, width, N, isign):
    for i in range(width):
        if(isign == -1): # Forward
            ...
            test2D = changeAmplitudeSpatial(test2D, width)
...
def ApplyFFTCol(pixels, height, N, isign):
    for i in range(height):
        if(isign == -1): # Forward
            ...
            test2D = changeAmplitudeSpatial(test2D, height)
...
def changeAmplitudeSpatial(array, Height):
